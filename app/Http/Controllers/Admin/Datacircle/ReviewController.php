...
class ReviewController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('Admin.Datacircle.Review.index',compact('reviews','user'))->with('no',1);
    }
    ...
    public function finish()
    {
        $user = Auth::user()->fname;
        $status = 4;
        $finishs = Task::where('project_id',1)->where(function($query) use ($status){
            $query->where('status',$status);
            })->paginate(20);
        return view('Admin.Datacircle.Review.finish',compact('finishs','user'))->with('no',1);
    }
    ...
    public function uploadtask()
    {
        $user = Auth::user()->fname;
        $status = 5;
        $uploads = Task::where('project_id',1)->where(function($query) use ($status){
            $query->where('status',$status);
            })->paginate(20);
        return view('Admin.Datacircle.Review.upload',compact('uploads','user'))->with('no',1);
    }
    ...
    public function getMoreReviews(Request $request)
    {
        ...
        if($request->ajax())
        {
            ...
            $no = 1;
            return view('Admin.Datacircle.Review.review_page',compact('reviews','user','no','search'))->render();
        }
    ...
    public function getMoreFinish(Request $request)
    {
        ...
        if($request->ajax())
        {
            ...
            $no = 1;
            return view('Admin.Datacircle.Review.finish_page',compact('finishs','user','no','search'))->render();
        }
    ...
    public function getMoreUpload(Request $request)
    {
        ...
        if($request->ajax())
        {
            ...
            $no = 1;
            return view('Admin.Datacircle.Review.upload_page',compact('uploads','user','no','search'))->render();
        }
