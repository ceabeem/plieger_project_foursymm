...
class IssueController extends Controller
{
    ...
    public function index()
    {
        ...
        $status = 6;
        $issues = Task::where('project_id',1)->where(function($query) use ($status){
            $query->where('status',$status);
        })->orderBy('issue_feedback','DESC')->orderBy('priority','DESC')->paginate(20);
        return view('Admin.Datacircle.Review.issue',compact('issues','user'))->with('no',1);
    }
    ...
    public function getMoreIssue(Request $request)
    {
        ...
        if($request->ajax())
        {
            ...
            $no = 1;
            return view('Admin.Datacircle.Review.issue_page',compact('issues','user','no','search'))->render();
        }
