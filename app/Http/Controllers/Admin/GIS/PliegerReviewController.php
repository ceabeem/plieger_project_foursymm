...
class PliegerReviewController extends Controller
{
    ...
    public function getMorePendings(Request $request)
    {
        ...
        if($request->ajax())
        {
            ...
            $no = 1;
            return view('Admin.GIS.ReviewPending.pending_page',compact('pendings','user','no','search'))->render();
        }
    ...
    public function getMorePliegerReview2(Request $request)
    {
        ...
        if($request->ajax())
        {
            $user = Auth::user()->fname;
            $status = 8;
            $feedbacks = Task::where('project_id',2)->where(function($query) use ($status){
                $query->where('status',$status);
                })->where(function($query) use ($search){
                            $query->where('task_name','like','%'.$search.'%')->orwhere('assigned_member','like','%'.$search.'%')->orwhere('assigned_date','like','%'.$search.'%');
                        })->paginate(10);
            $no = 1;
            return view('Admin.GIS.PliegerReview.pending_page',compact('feedbacks','user','no','search'))->render();
        }
    ...
    public function getMorePliegerReview1(Request $request)
    {
        ...
        if($request->ajax())
        {
            $user = Auth::user()->fname;
            $status = 7;
            $sends = Task::where('project_id',2)->where(function($query) use ($status){
                $query->where('status',$status);
                })->where(function($query) use ($search){
                            $query->where('task_name','like','%'.$search.'%')->orwhere('assigned_member','like','%'.$search.'%')->orwhere('assigned_date','like','%'.$search.'%');
                        })->paginate(10);
            $no = 1;
            return view('Admin.GIS.PliegerReview.pending_page2',compact('sends','user','no','search'))->render();
        }
