...
class TaskSummaryController extends Controller
{
    ...
    public function index()
    {
        ...
        $tasks =Task::where('project_id',2)->paginate(20);
        return view('Admin.GIS.Task.tasksummary',compact('tasks','user'))->with('no',1);
    }
    ...
    public function search(Request $request)
    {
            ...
            $data = Task::where('project_id',2)->where(function($query) use ($search){
        ...
        public function getMoreTasks(Request $request)
        {
            ...
            {
                $user = Auth::user()->fname;
                $tasks = Task::where('project_id',2)->where(function($query) use ($search){
                ...
                $no = 1;
                return view('Admin.GIS.Task.tasksummary_page',compact('tasks','user','no','search'))->render();
            }
        ...
        public function download(){
            ...
            $result1=DB::select(DB::raw("SELECT `id` as 'Task ID',`task_name` as 'Task Name',`assigned_member` as 'Assigned Member',`team_name` as 'Team Name',`review_assigned_member` as 'Review Assign Member',`status` as 'File_Status',`issue_remark`as 'Issue Remark' FROM `tasks` WHERE `project_id`=2"));
