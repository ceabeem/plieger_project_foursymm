...
class ReviewController extends Controller
{
    ...
    public function index()
    {
        ...
        $reviews = Task::where('status',2)->where(function($query) use ($team_name){
            $query->where('team_name',$team_name);
        })->where(function($query) use ($user){
            $query->where('review_assigned_member',$user)->orwhere('review_assigned_member',null);
        })->paginate(20);
        return view('TeamLeader.Review.index',compact('reviews','user'))->with('no',1);
    }
    ...
    public function getMoreReviews(Request $request)
    {
        ...
        if($request->ajax())
        {
            ...
            $no = 1;
            return view('TeamLeader.Review.review_page',compact('reviews','user','no','search'))->render();
        }
