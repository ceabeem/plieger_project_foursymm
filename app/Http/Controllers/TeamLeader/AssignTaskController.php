...
class AssignTaskController extends Controller
{
    ...
    public function index()
    {
        ...
        $tasks = Task::where('status',1)->where(function($query) use ($team_name){
            $query->where('team_name',$team_name);
        })->orderBy('id','DESC')->paginate(20);
        return view('TeamLeader.Task.assigntask',compact('tasks','members','user'))->with('no',1);
    }
    ...
    public function getallmembers()
    {
        ...
        $member = Member::where('user_id',$user_id)->first();
        $team_id = $member->team_id;
        $member_names =Member::where('role_id',2)->where(function($query) use ($team_id){
            $query->where('team_id',$team_id)
                ->where('flag',0);
        } )->get();
        $status = 1;
    ...
    public function search(Request $request)
    {
        ...
        public function getMoreTasks(Request $request)
        {
            ...
            if($request->ajax())
            {
                ...
                $no = 1;
                return view('Admin.Task.task_page',compact('tasks','user','no','search'))->render();
            }
